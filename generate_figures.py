...
def fig_vanilla_vs_optimized():
    ...
    ax.bar(x + w, optim, w, color=C_BLUE, label="Optimized GPTQ 4-bit", edgecolor="white")
    ...
    ax.legend(loc="upper right", framealpha=0.9)
    ax.set_title(
        "GPTQ 4-bit: Vanilla vs Optimized (g=128 + act-order + true-seq)",
        fontsize=13,
        fontweight="bold",
    )
...
def fig_gptq_vs_rtn():
    ...
    ax.annotate(
        ...
        fontweight="bold",
        color=C_RED,
        arrowprops={"arrowstyle": "->", "color": C_RED, "lw": 2},
    )
...
def fig_extreme_quantization():
    ...
    # Annotate key points
    for b, v in zip(bits, gptq, strict=True):
        ax.annotate(
...
def fig_algorithm_diagram():
    ...
    for i, (bx, text) in enumerate(boxes):
        ...
        if i < len(boxes) - 1:
            ...
            ax.annotate(
                "",
                xy=(next_bx - 0.05, box_y + box_h / 2),
                xytext=(bx + box_w + 0.05, box_y + box_h / 2),
                arrowprops={"arrowstyle": "-|>", "color": "#7f8c8d", "lw": 2},
            )
