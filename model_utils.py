...
Supports GPT-2, LLaMA, Mistral, Qwen2, OPT and other architectures via arch_config.
"""
...
# transformers >= 4.56 renamed the `torch_dtype` argument of `from_pretrained`
# to `dtype`; the old name still works but emits a deprecation warning, and it
# is removed entirely in later releases. Pick the right keyword for the
# installed version so this stays warning-free and forward-compatible.
_DTYPE_KWARG = (
    "dtype"
    if _pkg_version.parse(_transformers_version) >= _pkg_version.parse("4.56.0")
    else "torch_dtype"
)
...
def load_model(model_name="gpt2", device="cuda", token=None):
    ...
    model = AutoModelForCausalLM.from_pretrained(model_name, token=token, **{_DTYPE_KWARG: dtype})
