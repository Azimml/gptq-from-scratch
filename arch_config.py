...
Supports GPT-2, LLaMA (covers Llama-2/3), Mistral, Qwen2, and OPT model
families. Each family is described by an :class:`ArchConfig` whose callables
abstract away the parts of the forward pass that differ between architectures.
"""
...
def _unwrap_block_output(out):
    """
    Return the hidden-state tensor from a transformer block's output.

    Different HuggingFace versions/architectures return the hidden state either
    directly as a tensor or wrapped in a tuple/`ModelOutput`. Older transformers
    (< 5.0) returned a tuple ``(hidden, ...)`` for GPT-2/OPT blocks; transformers
    5.x returns a bare tensor. This helper handles both so error propagation
    between blocks stays correct.
    """
    if isinstance(out, torch.Tensor):
        return out
    if isinstance(out, (tuple, list)):
        return out[0]
    # transformers ModelOutput (dataclass-like) exposes `.last_hidden_state`
    hidden = getattr(out, "last_hidden_state", None)
    if hidden is not None:
        return hidden
    raise TypeError(f"Unexpected block output type: {type(out)}")
...
class ArchConfig:
    ...
    sublayer_groups: list[list[str]] | None = field(default=None)  # for true-sequential
...
def _gpt2_block_forward(block, hidden, **kwargs):
    return _unwrap_block_output(block(hidden, **kwargs))
...
# ---------------------------------------------------------------------------
# Qwen2 (covers Qwen2 / Qwen2.5 dense causal LMs)
# ---------------------------------------------------------------------------
#
# Qwen2 is architecturally very close to LLaMA: the same `model.layers` /
# `model.norm` / `model.embed_tokens` layout, RoPE via `model.rotary_emb`, GQA,
# and identical q/k/v/o + gate/up/down projection names. The only structural
# difference is that Qwen2's attention projections carry biases, but GPTQ only
# touches the `.weight` of each Linear, so the biases are left intact and the
# LLaMA accessors apply unchanged. A dedicated config is kept for clarity and
# so the supported-architecture list is explicit.
...
def get_arch_config(model) -> ArchConfig:
    ...
    raise ValueError(
        f"Unsupported architecture: {config_name}. Supported: {list(_CONFIG_CLASS_MAP.keys())}"
    )
