...
Supports GPT-2, LLaMA, Mistral, Qwen2, OPT and other architectures via
arch_config.
...
def _capture_and_quantize(
    ...
    # layer_H / layer_n go out of scope on return; no explicit `del` (which
    # would make them function-locals and confuse static analysis of the hook
    # closure above).
...
def quantize_model(
    ...
    Args:
        model: HuggingFace causal LM (GPT-2, LLaMA, Mistral, Qwen2, OPT, etc.).
    ...
    for block_idx, block in enumerate(tqdm(blocks, desc=f"GPTQ {n_bits}-bit blocks")):
        ...
        for h in hidden_states:
            new_hidden.append(
                arch.block_forward(block, h.to(device), **block_kwargs).detach().cpu()
            )
